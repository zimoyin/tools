 ...
 * @date : 2023/12/08
 *
 * 上下文数据类，用于存储和操作执行过程中的数据
 */
open class MapContent : Serializable {
    /**
     * 存储上下文数据,该字段是内部使用字段，不建议外部使用
     */
    ...
    val keys
        get() = contentInternal.keys

    val values
        get() = contentInternal.values

    fun keys(): MutableSet<String> {
        return contentInternal.keys
    }

    fun values(): MutableCollection<Any?> {
        return contentInternal.values
    }

    fun toMap(): Map<String, Any?> {
        return contentInternal.toMap()
    }

    inline fun filter(predicate: (Map.Entry<String, Any?>) -> Boolean): Map<String, Any?> {
        return contentInternal.filter {
            predicate(it)
        }
    }

    fun <R> map(transform: (Map.Entry<String, Any?>) -> R): List<R> {
        return contentInternal.map {
            transform(it)
        }
    }
    ...
    override fun toString(): String {
        return contentInternal.keys.toString()
    }
}
